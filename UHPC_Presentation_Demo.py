...
def property_prediction_demo():
    ...
    scenario = st.sidebar.selectbox(
        ...
        ["Custom Parameters", "Standard UHPC", "High-Strength Bridge", "Blended Mix", "Cost-Optimized"]
    )
...
def mix_comparison_demo():
    ...
    comparison_type = st.selectbox(
        ...
        ["Standard vs UHPC vs Blended", "Strength Optimization", "Cost vs Performance", "Custom Comparison"]
    )
    
    target_age = st.slider("Target Age for Comparison (days)", 1, 365, 28, 1)
    
    # Predefined mix comparisons for presentation
    if comparison_type == "Standard vs UHPC vs Blended":
        mixes = [
            {
                'name': 'Standard UHPC',
    ...
    with col1:
        ...
        fig = make_subplots(
            ...
            subplot_titles=('Compressive Strength Comparison', 'Cost Analysis', 
                           'SCM Content (%)', 'Performance vs Cost'),
    ...
    with col2:
        ...
        for i, result in enumerate(sorted_results):
            ...
            with st.container():
                ...
                st.markdown(f"Cost: £{result['Cost (£/m³)']}/m³")
